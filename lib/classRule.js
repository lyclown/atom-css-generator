...
class ClassRule {
    ...
    generate(className) {
        for (const rule of this.rules) {
            const cssRule = rule.call(this, className);
    ...
    internallyClassName(className) {
        const internallyClass = new Map([
            ['te-ellipsis', `white-space: nowrap;
            overflow: hidden;
            text-overflow: ellipsis;`],
            ['.te-ellipsis-2', `display: -webkit-box;
            -webkit-line-clamp: 2;
        ...
        return internallyClass.get(className) ? `{${internallyClass.get(className)}}` : '';
    }
}
